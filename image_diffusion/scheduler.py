...
class DDPMScheduler(BaseScheduler):
    def __init__(self, num_train_timesteps, beta_1, beta_T, mode="linear", sigma_type="small"):
        ...
        if sigma_type == "small":
            ...
            sigmas = (1 - alphas_cumprod_t_prev) / (1 - self.alphas_cumprod) * self.betas
    ...
    def add_noise(self, original_sample, timesteps: torch.IntTensor, noise: Optional[torch.Tensor] = None):
        """
        Input:
            sample: [B,C,H,W]
            timesteps: [B]
        ...
        noisy_sample = sqrt_alpha_prod * original_sample + one_minus_sqrt_alpha_prod * noise
...
class DDIMScheduler(BaseScheduler):
    ...
    def set_timesteps(self, num_inference_timesteps: int, device: Union[str, torch.device] = None):
        if num_inference_timesteps > self.num_train_timesteps:
            raise ValueError(f"num_inference_timesteps ({num_inference_timesteps}) cannot exceed self.num_train_timesteps ({self.num_train_timesteps})")
    ...
    def step(self, sample: torch.Tensor, timestep: int, noise_pred: torch.Tensor, eta: float = 0.0):
        timestep_prev = timestep - self.num_train_timesteps // self.num_inference_timesteps
        alpha_prod_t = self.alphas_cumprod[timestep]
        alpha_prod_t_prev = self.alphas_cumprod[timestep_prev] if timestep_prev >= 0 else self.alpha_prod_0
        ...
        sample_prev = torch.sqrt(alpha_prod_t_prev) * pred_original_sample + direction_to_sample * noise_pred + random_noise
